...
for epoch in range(args.n_steps):
    ...
    c_loss_a+=loss_a.data
    c_loss_a=c_loss_a/len(dataloader_a)
    plot_a_loss.append(c_loss_a)
    c_loss_b+=loss_b.data
    c_loss_b=c_loss_b/len(dataloader_b)
    plot_b_loss.append(c_loss_b)
    if epoch % 1000 == 0:
        print(to_print)
...
plt.plot(range(1, args.n_steps+1),plot_a_loss,'-',color='green',linewidth=3,label='Loss when classifying A')
plt.plot(range(1, args.n_steps+1),plot_b_loss,'-',color='red',linewidth=3,label='Loss when classifying B')
plt.xlabel('epoch')
plt.ylabel('loss')
plt.grid(True)
plt.legend()
